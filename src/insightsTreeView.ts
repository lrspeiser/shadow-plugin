...
export class InsightsTreeProvider implements vscode.TreeDataProvider<TreeItem> {
    ...
    private getRootItems(): TreeItem[] {
        ...
        const genDocsBtn = new TreeItem('Generate Product Documentation', vscode.TreeItemCollapsibleState.None);
        genDocsBtn.type = 'action';
        
        // Check if analysis is required and complete
        const canRunDocs = this.analysisStatus === 'complete';
        
        // Show visual feedback based on status
        if (this.productDocsStatus === 'generating') {
            genDocsBtn.label = 'Generating Product Documentation...';
            genDocsBtn.iconPath = new vscode.ThemeIcon('sync~spin');
            genDocsBtn.description = 'In progress...';
            genDocsBtn.command = undefined; // Disable button while generating
        } else if (this.productDocsStatus === 'complete') {
            genDocsBtn.label = 'Generate Product Documentation';
            genDocsBtn.iconPath = new vscode.ThemeIcon('check', new vscode.ThemeColor('charts.green'));
            const lastRun = this.formatTimestamp(this.productDocsTimestamp);
            genDocsBtn.description = lastRun ? `Last run: ${lastRun}` : '✅ Ready';
            genDocsBtn.command = {
                command: 'shadowWatch.generateProductDocs',
                title: 'Generate Product Docs'
            };
        } else if (!canRunDocs) {
            genDocsBtn.iconPath = new vscode.ThemeIcon('book');
            genDocsBtn.description = 'Run "Analyze Workspace" first';
            genDocsBtn.command = undefined; // Disabled - gray out
            // Make it appear grayed out
            genDocsBtn.contextValue = 'disabled';
        } else {
            genDocsBtn.iconPath = new vscode.ThemeIcon('book');
            genDocsBtn.description = 'Click to generate';
            genDocsBtn.command = {
                command: 'shadowWatch.generateProductDocs',
                title: 'Generate Product Docs'
            };
        ...
        items.push(genDocsBtn);

        const genInsightsBtn = new TreeItem('Generate AI Architecture Insights', vscode.TreeItemCollapsibleState.None);
        genInsightsBtn.type = 'action';
        
        // Check prerequisites
        const canRunInsights = this.analysisStatus === 'complete' && 
                              (this.productDocsStatus === 'complete' || this.insightsStatus === 'complete');
        
        // Check if insights are generating
        if (this.insightsStatus === 'generating') {
            genInsightsBtn.label = 'Generating AI Architecture Insights...';
            genInsightsBtn.iconPath = new vscode.ThemeIcon('sync~spin');
            genInsightsBtn.description = 'In progress...';
            genInsightsBtn.command = undefined; // Disable button while generating
        } else if (this.insightsStatus === 'complete') {
            genInsightsBtn.label = 'Generate AI Architecture Insights';
            genInsightsBtn.iconPath = new vscode.ThemeIcon('lightbulb');
            const lastRun = this.formatTimestamp(this.insightsTimestamp);
            genInsightsBtn.description = lastRun ? `Last run: ${lastRun}` : '✅ Ready';
            // Always enable if complete (product docs requirement is for first generation)
            genInsightsBtn.command = {
                command: 'shadowWatch.generateLLMInsights',
                title: 'Generate AI Insights'
            };
        } else if (!canRunInsights) {
            // Determine why it's disabled
            if (this.analysisStatus !== 'complete') {
                genInsightsBtn.iconPath = new vscode.ThemeIcon('lightbulb');
                genInsightsBtn.description = 'Run "Analyze Workspace" first';
                genInsightsBtn.contextValue = 'disabled';
            } else if (this.productDocsStatus === 'generating') {
                genInsightsBtn.iconPath = new vscode.ThemeIcon('lightbulb');
                genInsightsBtn.description = 'Wait for docs to complete...';
                genInsightsBtn.contextValue = 'disabled';
            } else {
                genInsightsBtn.iconPath = new vscode.ThemeIcon('lightbulb');
                genInsightsBtn.description = 'Generate product docs first';
                genInsightsBtn.contextValue = 'disabled';
            }
            genInsightsBtn.command = undefined; // Disabled
        } else {
            genInsightsBtn.iconPath = new vscode.ThemeIcon('lightbulb');
            genInsightsBtn.description = 'Ready';
            genInsightsBtn.command = {
                command: 'shadowWatch.generateLLMInsights',
                title: 'Generate AI Insights'
            };
        }
        items.push(genInsightsBtn);
